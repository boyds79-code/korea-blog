...
const BLOG_DIR = path.join(ROOT, 'src', 'content', 'beauty');
...
async function main() {
  ...
  const imagesDir = path.join(ROOT, 'public', 'images', 'beauty', slug);
  ...
  const frontmatter = [
    ...
    ...(cover
      ? [
          `heroImage: ${yamlString(`/images/beauty/${slug}/${cover.filename}`)}`,
  ...
  if (needsManual.length > 0) {
    ...
    checklist = [
      ...
      someAuto
        ...
        : `🎨 이 글에 필요한 AI 생성 이미지 (머지 전에 준비해서 넣어주세요) — 아래 프롬프트로 AI가 생성한 이미지를 씁니다.`,
      `무료 생성 도구: https://www.bing.com/images/create (Microsoft 계정만 있으면 무료) — 아래 프롬프트를 그대로 복사해서 붙여넣으세요.`,
      `생성한 이미지는 public/images/beauty/${slug}/ 폴더 안에 아래 파일명 그대로 저장하면 자동으로 연결됩니다.`,
    ...
    checklist = [
      '<!--',
      `✅ 이미지 ${imagePlan.length}장 모두 AI로 자동 생성되어 이미 폴더에 들어가 있습니다 (public/images/beauty/${slug}/). 내용만 확인하고 머지하면 됩니다.`,
      '-->',
      '',
    ].join('\n');
  }
  ...
  if (needsManual.length > 0) {
    console.log(`[generate-post] 직접 준비해야 할 이미지 ${needsManual.length}장 (public/images/beauty/${slug}/ 안에 넣어주세요):`);
