...
// public/images/recipes/<slug>/ 폴더를 자동 생성 → Finder로 열어줌 →
...
async function main() {
  ...
  // 4. 이미지 폴더 생성
  const targetDir = path.join('public', 'images', 'recipes', slug);
  fs.mkdirSync(targetDir, { recursive: true });
  console.log(`📁 폴더 준비 완료: ${targetDir}`);

  // 5. 이 글에 필요한 이미지 체크리스트 보여주기 + 필요한 파일명 정확히 추출
  const mdPath = path.join('src', 'content', 'recipes', `${slug}.md`);
  let requiredFiles = [];
