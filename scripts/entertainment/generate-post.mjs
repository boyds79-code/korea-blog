...
const BLOG_DIR = path.join(ROOT, 'src', 'content', 'entertainment');
...
async function main() {
  ...
  const imagesDir = path.join(ROOT, 'public', 'images', 'entertainment', slug);
  ...
  const frontmatter = [
    ...
    ...(cover
      ? [
          `heroImage: ${yamlString(`/images/entertainment/${slug}/${cover.filename}`)}`,
  ...
  if (needsManual.length > 0) {
    ...
    checklist = [
      ...
      `생성한 이미지는 public/images/entertainment/${slug}/ 폴더 안에 아래 파일명 그대로 저장하면 자동으로 연결됩니다.`,
    ...
    checklist = [
      '<!--',
      `✅ 이미지 ${imagePlan.length}장 모두 AI로 자동 생성되어 이미 폴더에 들어가 있습니다 (public/images/entertainment/${slug}/). 내용만 확인하고 머지하면 됩니다.`,
      '-->',
      '',
    ].join('\n');
  }
  ...
  if (needsManual.length > 0) {
    console.log(`[generate-post] 직접 준비해야 할 이미지 ${needsManual.length}장 (public/images/entertainment/${slug}/ 안에 넣어주세요):`);
