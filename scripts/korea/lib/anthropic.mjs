...
- For images #2 and onward, embed a real Markdown image tag directly in body_markdown at the point in the article where that image would help most, using EXACTLY this path pattern: ![alt text](/images/korea/${SLUG_PLACEHOLDER}/N.jpg) where N is the image's position number (2, 3, 4...). Use the literal text "${SLUG_PLACEHOLDER}" — do not invent a slug yourself.
